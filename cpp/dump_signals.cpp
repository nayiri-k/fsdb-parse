...
{
    fprintf(stderr, "Size of buffer type: %u\n",
        sizeof(BUFFER_TYPE));
    
    if (!((4 <= argc) && (argc <= 6))) {
    fprintf(stderr, "argc: %u\n",argc);
	fprintf(stderr, "usage: ds verilog_type.fsdb idcodes.txt out_toggles.bin [start_time [end_time]]\n");
    ...
    for (i = 0; i < N_ROWS; i++) {
        ...
        // fprintf(stderr, "Traversing signal %u", idcode);
        ...
        // fsdbTag64 time_window;
        
        // time_window.L = TW_L + start_time;
        idx = 0;
        // zero out buffer
        ...
        do {
            ...
            if (current_simtime <= start_time) {
                ...
                while (current_simtime - TW_L > time_window) {
                    toggle_buff[idx] = (BUFFER_TYPE) sig_val;
                ...
                switch(vc_ptr[0]) { // assume each signal is 1-bit
                    case FSDB_BT_VCD_0:
                        sig_val = 0;
                        break;
