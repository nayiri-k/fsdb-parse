...
BUFFER_TYPE* sigtrans_buff;
BUFFER_TYPE* sigvals_buff;
...
main(int argc, char *argv[])
{
	...
	fprintf(stderr, "usage: ds verilog_type.fsdb idcodes.txt sigwidths.txt out_sigtrans.bin out_sigvals.bin [start_time [end_time]]\n");
    ...
    char *fpath_sigwidths = argv[3];
    char *fpath_sigtrans = argv[4];
    char *fpath_sigvals = argv[5];
    ulong_T start_time = 0;
    ulong_T end_time = 0;
    if (argc >= 6) {
        start_time = atoi(argv[6]);
    }
    fprintf(stderr, "Start time: %lu\n",start_time);
    if (argc >= 7) {
        end_time = atoi(argv[7]);
    }
    ...
    FILE *f_sigtrans = fopen(fpath_sigtrans, "wb");
    if (f_sigtrans == NULL) {
        fprintf(stderr, "ERROR: File could not be created, %s\n", fpath_sigtrans);
        return 0;
    }

    FILE *f_sigvals = fopen(fpath_sigvals, "wb");
    if (f_sigvals == NULL) {
        fprintf(stderr, "ERROR: File could not be created, %s\n", fpath_sigvals);
        return 0;
    }
    ...
    sigtrans_buff = (BUFFER_TYPE*) malloc(M_COLS*sizeof(BUFFER_TYPE));
    value_buff = (BUFFER_TYPE*) malloc(M_COLS*sizeof(BUFFER_TYPE));
    ...
    fwrite(&N_ROWS, sizeof(uint_T), 1, f_sigtrans);
    fwrite(&M_COLS, sizeof(uint_T), 1, f_sigtrans);

    fwrite(&N_ROWS, sizeof(uint_T), 1, f_sigvals);
    fwrite(&M_COLS, sizeof(uint_T), 1, f_sigvals);
    ...
    for (i = 0; i < N_ROWS; i++) {
        ...
        for (bit_idx=msb; bit_idx <= msb; bit_idx--) { // after 0 bit_idx will roll over to huge #
            ...
            uint_T num_sigtrans = 0;  // range is 0-N_CYCLES, or 0-2*N_CYCLES for clk (R+F edge)
            uint_T num_sigones = 0;
            
            ulong_T time_window = TW_L + start_time*TIMESCALE;
            ulong_T current_simtime = 0;
            ulong_T prev_simtime = 0;
            ...
            idx = 0;
            // zero out buffer
            memset(sigtrans_buff, (BUFFER_TYPE) 0, M_COLS*sizeof(BUFFER_TYPE));
            memset(sigvals_buff, (BUFFER_TYPE) 0, M_COLS*sizeof(BUFFER_TYPE));
            do {
                vc_trvs_hdl->ffrGetXTag(&time);
                vc_trvs_hdl->ffrGetVC(&vc_ptr);

                // ulong_T time_upper = (ulong_T) (time.H)
                current_simtime = ((ulong_T) time.H << 32) + time.L;
                // initial state (don't include in # toggles)
                if (current_simtime == 0) {
                ...
                if (current_simtime <= start_time) {
                    continue;
                } else if (current_simtime > end_simtime) {
                    // fprintf(stderr, "ERROR: simtime exceeds end time (curr_simtime: %u, end_simtime: %u)\n", 
                    //             (uint_T) current_simtime, end_simtime);
                    break;
                } else if (current_simtime >= time_window) {
                    // signal was one for the remainder of the previous time window
                    if (prev_bit_val == FSDB_BT_VCD_1)   { 
                        num_sigones += (uint_T) (time_window - prev_simtime)/CLOCK_TICK;
                        if (num_sigones > TW_TICKS) num_sigones = TW_TICKS;
                    }
                    while (current_simtime - TW_L > time_window) {
                        time_window += TW_L; idx += 1;
                    }

                    if (idx >= M_COLS) {
                        fprintf(stderr, "ERROR: index exceeds number of columns (idx: %u, curr_simtime: %u, end_time: %u)\n", 
                                idx, (uint_T) current_simtime, end_time);
                        return 0;
                    }
                        
                    

                    sigtrans_buff[idx] = (BUFFER_TYPE) num_sigtrans;
                    sigvals_buff[idx] = (BUFFER_TYPE) num_sigones;
                        // /N_CLOCK_CYCLES if float, so range should be 0-N_CLOCK_CYCLES
                        //      or 0-2*N_CLOCK_CYCLES for clk (rising + falling)
                    // all_zeros = false;
                    
                    // now time window includes current value change
                    // reset num_sigtrans/num_sigones
                    time_window += TW_L; idx += 1;
                    num_sigtrans = 1; // include current value change (which is now in time window)
                    num_sigones = 0; // only count ones when prev signal == 1
                } else {
                    num_sigtrans += 1;
                    uint_T prev_time = time_window - TW_L;  // 1 was from previous window
                    if (prev_time < prev_simtime) prev_time = prev_simtime; // 1 was from previous value change
                    if (prev_bit_val == FSDB_BT_VCD_1) num_ones += (uint_T) (time.L - prev_simtime)/CLOCK_TICK;
                }
                prev_bit_val = vc_ptr[bit_idx];
                prev_simtime = current_simtime;
            ...
            fwrite(sigtrans_buff, sizeof(BUFFER_TYPE), M_COLS, f_sigtrans);
            fwrite(sigvals_buff, sizeof(BUFFER_TYPE), M_COLS, f_sigvals);
            // if (!all_zeros) {
            //     // write data to buffer
            //     fwrite(sigtrans_buff, sizeof(BUFFER_TYPE), M_COLS, f_sigtrans);
            // }        
